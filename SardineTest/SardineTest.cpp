...
//		LOGW("Loading function failed: %s",L#name); \
...
//                _tprintf(TEXT("Found J2534 device: (%d) %s\n"), i+1, achKey);
	...
	for (i=0; i<cValues; i++) 
        { 
            ...
            if (ret == ERROR_SUCCESS) 
            {
				...
				{
				LOGW(L"Sardine CAN: %d: [%s] = \"%s\"", i+1, achValueName,achValue);
				} else if (valueType==REG_DWORD)
				{
				DWORD val = (achValue[3]<<24) + (achValue[2]<<16) + (achValue[1]<<8) + achValue[0];	// little endian
				LOGW(L"Sardine CAN: %d: [%s] = [%d]", i+1, achValueName,val);
				} else {
				LOGW(L"Sardine CAN: %d: [%s] = *** currently unsupported type! ***", i+1, achValueName);
				} 
            }
        }
	/*
	...
	*/
	...
	if (hDLL==NULL)
		{
		LOG("Loading failed, loading library #2.. (c:\\sardine\\sardine.dll)");
		hDLL = LoadLibrary(_T("c:\\sardine\\sardine.dll"));
		}


//	hDLL = LoadLibrary(_T("c:\\program files\\dice\\tools\\tsdice64.dll"));
//	hDLL = LoadLibrary(_T("c:\\windows\\system32\\vccpt32.dll"));
//	hDLL = LoadLibrary(_T("c:\\temp\\sardin32.dll"));
	if (hDLL==NULL)
	{
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	LOG("Sardine test util v1.10 : (c) Olaf @ HackingVolvo Labs 2012");
